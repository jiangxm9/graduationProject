 ...
 */
function verifyResPassword($username, $password)
{
    $hash = getResPasswdHash($username);
 ...
 * @return string|bool 成功时返回Token，失败返回false
 */
function verifyEmpPassword($username, $password)
{
    $hash = getEmpPasswdHash($username);
    if (!$hash) return false;
 ...
 * @return string 生成的Token
 */
function registRes($username, $password)
{
    $passwd = passwordHash($password);
    addRestaurant($username, $passwd);
 ...
 * @return int 店员的id
 */
function registEmp($username, $password)
{
    $passwd = passwordHash($password);
    addEmployee($username, $passwd);
    return getEmpIdByUsername($username);
}
 ...
 */
function getResId($username)
{
    if ($username === false) return false;
    return (int)getResIdByUsername($username);
}
...
{
    if ($username === false) return false;
    return (int)getEmpIdByUsername($username);
}
 ...
 * @return int 成功时返回饭店id，失败返回false
 */
function getResIdByEmp($empid)
{
    if ($empid == 0) return 0;
    return (int)getResIdByEmpId($empid);
}
